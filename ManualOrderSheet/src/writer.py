...
def render(instruction: Instruction, output_dir: Path,
    ...
    for idx, line in enumerate(instruction.lines):
        ...
        set_cell_text(cells[1], line.fund_label)
        set_cell_text(cells[2], f"{line.apply_date:%Y-%m-%d}")
        set_cell_text(cells[3], f"{line.qty:,}주")
        set_cell_text(cells[4], f"{line.amount:,}원")
        set_cell_text(cells[5], f"{line.trade_name}\n{line.detail}")
