...
# 문서 종류별 템플릿 (instruction.DIVIDEND / ODD_LOT 키)
TEMPLATE_PATHS = {
    "dividend": TEMPLATE_DIR / "수기운용지시서_배당지급_template.docx",
    "odd_lot": TEMPLATE_DIR / "수기운용지시서_단주대금_template.docx",
}
TEMPLATE_PATH = TEMPLATE_PATHS["dividend"]   # 하위 호환
...
# --- 배당지급 문서: (운용구분, 권리구분, 증권구분) -> 거래유형명 ---
# 권리배정내역의 '대금' 이 0 보다 큰 행이 대상. 등록 안 된 조합은 문서를 만들지 않고
# 건너뛴다(잘못된 공문이 나가는 것보다 안전). 실제로 써 본 유형만 등록한다.
TRADE_TYPE_NAMES = {
    ("차입", "현금배당", "주식"): "차입주식 배당지급",
}

# --- 단주대금 문서: (운용구분, 증권구분) -> 거래유형명 ---
# 권리배정내역의 '단주대금' 이 0 보다 큰 행이 대상. 주식배당·회사분할 등 권리구분과
# 무관하게 같은 문구를 쓴다.
ODD_LOT_TRADE_NAMES = {
    ("차입", "주식"): "차입주식 단주대금 지급",
}
