...
| `--ipo-ticker` | IPO 종목 티커 추가 지정 (여러 번 사용 가능) |
| `--ipo-auto` | IPO 시트 미등록 무상입고 종목도 IPO 로 자동 편입 |
...
Prelude 에서 IPO 배정주는 **대금 0원 `BUY LONG`(무상입고)** 으로 들어오고,
청약대금은 별도 **`Wires`** 로 빠져나갑니다. 따라서

```
IPO 손익 = (배정주 평가액 변동 + 매도대금) − 청약대금
```

- IPO 종목은 `EQSWAP.xlsx` 의 `IPO` 시트를 마스터로 삼습니다.
- 청약대금 Wire 는 그 시트의 `Payment Amount`(원화) 와 금액을 대조해 찾아냅니다.
  (예: Justek 18,937,500원 ↔ 2026-06-23 Wire 정확히 일치)
- 시트에 없는 무상입고는 `08_IPO후보` 에 보고만 하고 현물에 남깁니다.
  IPO 가 맞으면 `--ipo-auto` 또는 `--ipo-ticker` 로 편입하세요.

**주의**: 청약대금이 컷오버 이전에 나가고 손익은 이후에 실현되는 건은
`00_요약` 의 "컷오버 이전 지급 청약대금(참고)" 에 표시만 합니다.
Prelude 장부상 5/29 시점 평가액이 이미 0 이라 6월 손익에서 차감하면 이중계상이 되기 때문입니다.
(해당 건: 2026-05-29 Piece Peace Studio 9,222.51 USD)
