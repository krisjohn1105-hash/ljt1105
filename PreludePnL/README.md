...
| `12_Report_Availability` | 기준일별 리포트 도착 현황 / 필수 리포트 누락 여부 |
...
리포트를 받아둔 뒤 이 한 줄만 실행하면 됩니다.

```bash
python daily_pnl.py --src "Z:/02.펀드/003.매매보고서 대사/Prelude_new"
```

실행하면 순서대로 (1) 리포트 도착 현황 점검 → (2) 손익 계산·엑셀 갱신 →
(3) **루트에 새로 들어온 리포트를 대분류 폴더로 정리**까지 한 번에 처리합니다.
(`--no-organize` 로 정리만 끌 수 있습니다.)

### 리포트가 덜 도착한 날은 계산하지 않습니다

Prelude 리포트는 아침에 나눠서 들어옵니다. 평가(`MAC001X`)만 오고
거래(`MAC002TDX`)가 아직 안 온 상태로 계산하면 자산군 배분이 망가지고,
반대로 평가가 빠지면 그 다음날 손익이 이틀치 평가변동에 하루치 현금흐름만
붙어 **총액까지 틀립니다**.

그래서 필수 리포트(`MAC001X`, `MAC002TDX`)가 다 오지 않은 기준일은
계산에서 빼고, **그 다음 영업일도 산출 대상에서 제외**합니다.
콘솔에 경고가 뜨고 `12_Report_Availability` 시트에서 날짜별 도착 현황을 볼 수 있습니다.

```
      최신 리포트일 2026-09-02 (1일 전) · 오늘 2026-09-03
      ! 2026-09-02 필수 리포트 미도착(MAC002TDX) → 산출 제외
```

제외된 날은 다음 실행 때 리포트가 채워지면 자동으로 다시 계산됩니다.
`00_Summary` 의 `Latest report date` / `Latest complete report date` 로
지금 보고 있는 숫자가 며칠자인지 확인하세요.
